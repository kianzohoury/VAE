...
model such process with a neural network? Yes. In fact, generative models of the 
form p_{θ}(x) try to find an optimal $θ$ are designed exactly for the purpose of generating new data points 
that resemble those from $X$.
